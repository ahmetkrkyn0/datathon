...
### Ikinci tercih: daha muhafazakar
...
Finalde iki cozum secilebiliyorsa bu iki dosya birlikte iyi bir risk dagilimi
sagliyor. Tek hak varsa aggressive aday denenebilir.

## Arkadasa Yollanacak Dosyalar

### Minimum paket

Arkadas sadece submission'lari inceleyecek veya yarismaya yukleyecekse:
...
OOF sonuclarini kontrol edecekse minimum pakete sunlari ekle:
...
### Tam yeniden uretim paketi

Scripti bastan calistiracaksa yukaridakilere ek olarak:

```text
ourteam_oof_tunafolds.npy
ourteam_test_tunafolds.npy
fullft_oof.npy
fullft_test.npy
ftt_oof.npy
ftt_test.npy
...
tunadanistenenbelgeler/oof_blend.npy
tunadanistenenbelgeler/test_blend.npy

tunadan gelenler/models.npy
tunadan gelenler/y.npy
tunadan gelenler/w_recency.npy
tunadan gelenler/student_id_train.npy
tunadan gelenler/student_id_test.npy
tunadan gelenler/folds.parquet
tunadan gelenler/oof_*.npy
tunadan gelenler/test_*.npy
```

Tam yeniden uretim komutu:
...
## Yollanmamasi Onerilen Dosya

```text
submissions/SUB_combinatorial_plus_mmstrong.csv
```

Bu dosya `combinatorial` tahminine dogrudan `%6 mmstrong` karistiriyordu.
Kontrolde `mmstrong` 12-model nested stack'i:

```text
82.2397 -> 82.2821
```

seklinde kotulestirdi. Bu nedenle yeni TARGET815 adaylarinin gerisinde kaldi.
