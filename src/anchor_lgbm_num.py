...
FEATURE UZAYI (SPEC §4 Adim 7: "sadece sayisal + temel FE + kategorik"; kullanici onayi):
  lgbm_num = sayisal (37) + native-kategorik (5) + missing-flag (7) = 49 feature. METIN YOK.
  ("num" = yapisal/metinsiz; lgbm_full = lgbm_num + NLP, Faz 05.)
...
ANCHOR REFERANS NOTU (kullanici karari + reports/roadmap-adversarial-review.md M1):
  SPEC/prompt'taki "~91.6" referansi YENIDEN URETILEMIYOR (numeric-only ~93-94,
  num+kategorik ~88-89; FE eklemek 89.97'nin altina indirir, ustune cikaramaz). Bu altyapi
  hatasi DEGIL; "91.6" etiketi stale. Bu kosuda OLCULEN deger downstream kabul kapisi
  (0.25*std) tabani olarak kilitlenir; "~91.6" yalnizca tarihsel hedef etiketidir.
  Olculen mean ~88, EDA'nin yapisal baseline'i (89.86) ve review'in (89.97) ile uyumludur.
...
# Yapisal anchor icin makul referans bandi (91.6 stale; olculen ~88).
REF_LO, REF_HI = 85.0, 92.0
...
def main() -> None:
    ...
    print(f"[anchor] {MODEL}: {X.shape[1]} feature "
          f"({len(cv.numeric_feature_columns(train))} sayisal + {len(cv.NA_COLS)} flag + "
          f"{len(cat_features)} native-kategorik). Metin YOK.")
    ...
    note = (
        "lgbm_num = sayisal+native-kategorik+missing-flag (metin yok). "
        "cv_mse_mean/std = compute_cv_mse(oof) (avg-oof, DoD-4). 91.6 stale/yeniden-uretilemez "
        "(bkz review M1); OLCULEN deger downstream 0.25*std tabani. "
    ...
    if REF_LO <= cv_mean <= REF_HI:
        print(f"[anchor] cv_mse_mean {cv_mean:.2f} yapisal baseline bandinda ({REF_LO}-{REF_HI}); "
              f"EDA 89.86 / review 89.97 ile uyumlu -> ALTYAPI DOGRU. (91.6 stale, bkz review M1.)")
    else:
