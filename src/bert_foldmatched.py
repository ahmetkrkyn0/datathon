...
SEED = 42
N_FOLDS = 10  # GBM scriptleriyle ayni!
EPOCHS = 3
BATCH = 32
LR = 2e-5
MAX_LEN = 160
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(train)):
        ...
        out[f"emb_tr_f{fold}"] = e_tr
        out[f"emb_te_f{fold}"] = e_te
    ...
    print(f"\nBERT3 (10-fold) skalar OOF MSE = {((y_raw - oof100) ** 2).mean():.2f}")
    np.save(CACHE / "bert3_oof.npy", oof100)
    np.save(CACHE / "bert3_test.npy", te100)
    np.savez(CACHE / "bert_foldemb.npz", **out)
    print(f"KAYDEDILDI -> bert_foldemb.npz ({len(out)} dizi) + bert3_oof/test.npy")
