"""
submission_v8 — v7 + tuned XGB + LGBM-DART cesitliligi + 3 seed.

v7'den (LB 85.23) farklar:
  1. XGBoost artik tuned (tune_xgb.py -> best_params.json['xgb'])
  2. +LGBM-DART varyanti (dropout'lu boosting -> dekorele hatalar)
  3. ExtraTrees cikti (v7'de agirlik 0), MLP kaldi
  4. 3 seed (42, 7, 2024)
  5. Ayni: 10-fold, yil-bazli affine kalibrasyon, NNLS blend

Calistir: python -u src/train_model_v8.py
"""
...
OUT = ROOT / "submissions" / "submission_v8.csv"

SEED = 42
N_FOLDS = 10
SEEDS = [42, 7, 2024]
NAMES = ["lgbm", "dart", "xgb", "cat", "mlp"]
...
def main():
    ...
    cat_p, lgbm_p, xgb_p = (best["cat"]["params"], best["lgbm"]["params"],
                            best["xgb"]["params"])
    print(f"  sayisal {len(num_cols)} | modeller {NAMES} | {N_FOLDS}-fold x {len(SEEDS)} seed")
    ...
    def make(name, seed):
        ...
        if name == "dart":
            p = dict(lgbm_p)
            p["n_estimators"] = min(p["n_estimators"], 1000)
            return LGBMRegressor(**p, boosting_type="dart", drop_rate=0.1,
    ...
    np.savez(CACHE / "preds_v8.npz", y=y, w_fit=w_fit, years=yr_tr,
    ...
    print(f"\nYAZILDI -> {OUT}")
    print(f"LB tahmini ~{(cal_score if use_cal else base_score):.2f} "
          f"(proxy son zamanlarda ~0.5 muhafazakar)")
