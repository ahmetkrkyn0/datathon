...
Cikti: submission_v45.csv + submission_v46.csv (0.5*v15 + 0.5*v7)
...
OUT = ROOT / "submissions" / "submission_v45.csv"
OUT_MIX = ROOT / "submissions" / "submission_v46.csv"
...
def main():
    ...
    # F3: potansiyel tuzagi (guclu ovgu olmadan 'potansiyel' = kibar uyari)
    for df_, raw_path in [(train, "train.csv"), (test, "test_x.csv")]:
        t = pd.read_csv(ROOT / "data" / raw_path)["mentor_feedback_text"].fillna("").str.lower()
    ...
    num_cols = num_cols + ["nlp_potansiyel_tuzak"]
    ...
    np.savez(CACHE / "preds_v45.npz", y=y, w_fit=w_fit, years=yr_tr,
