...
Cikti: submission_v17.csv + submission_v18.csv (0.5*v15 + 0.5*v7)
...
OUT = ROOT / "submissions" / "submission_v17.csv"
OUT_MIX = ROOT / "submissions" / "submission_v18.csv"
...
def main():
    ...
    num_cols = num_cols + ["txt_bert", "txt_bert2"]
    # fine-tune edilmis BERT embedding -> PCA 64 (arkadas-notebook fikri,
    # kendi implementasyonumuz: OOF embedding, hedefe-gore-orgutlu uzay)
    from sklearn.decomposition import TruncatedSVD
    emb_tr = np.load(CACHE / "bert_emb_train.npy")
    emb_te = np.load(CACHE / "bert_emb_test.npy")
    esvd = TruncatedSVD(n_components=64, random_state=SEED)
    e_tr = esvd.fit_transform(emb_tr)
    e_te = esvd.transform(emb_te)
    for i in range(64):
        train[f"ftemb_{i}"], test[f"ftemb_{i}"] = e_tr[:, i], e_te[:, i]
    num_cols = num_cols + [f"ftemb_{i}" for i in range(64)]
    print(f"  ft-embedding PCA64 eklendi (acikl. var={esvd.explained_variance_ratio_.sum():.3f})")
    gmean = y.mean()
    ...
    np.savez(CACHE / "preds_v17.npz", y=y, w_fit=w_fit, years=yr_tr,
